...
<style>
  ...
  /* --- LAYOUT & SCROLL FIX --- */
  /* This ensures the page respects the footer's height calculated by the JS in admin_footer.php */
  .logs-page {
    /* No height setting needed here; it will use the default padding-bottom from the main layout */
  }
  .table-wrapper {
    overflow: auto; /* Enable scrollbars on this container */
    max-height: 55vh; /* KEY FIX: Constrain height relative to viewport height */
  }

  /* --- MAROON THEME & STYLES --- */
  .logs-toolbar h4 .fa-solid,
  .logs-page .nav-tabs .nav-link i {
    color: var(--g-maroon);
  }
  .logs-page .nav-tabs .nav-link {
    color: #495057;
  }
  ...
  .logs-page .nav-tabs .nav-link.active i {
    color: #fff;
  }
  ...
  .logs-page .btn-primary:hover, .logs-page .btn-primary:focus {
    background-color: var(--g-maroon-dark);
    border-color: var(--g-maroon-dark);
  }
  .logs-toolbar .btn-outline-primary {
    color: var(--g-maroon);
    border-color: var(--g-maroon);
  }
  .logs-toolbar .btn-outline-primary:hover {
    color: #fff;
    background-color: var(--g-maroon);
    border-color: var(--g-maroon);
  }
  /* Make action button in table maroon */
  ...
  <div class="tab-content border border-top-0 p-3 rounded-bottom bg-white">
    <div class="tab-pane fade show active" id="activity" role="tabpanel" aria-labelledby="activity-tab">
      <div class="row g-2 align-items-end mb-3 filters-row">
        ...
        <div class="col-md-3"><label class="form-label">Search</label><input type="text" id="act-search" class="form-control form-control-sm" placeholder="Search staff ID / username / action"></div>
        <div class="col-md-3 d-flex justify-content-start align-items-end"><button id="act-filter" class="btn btn-primary btn-sm"><i class="fa-solid fa-filter me-1"></i> Apply</button><button id="act-reset" class="btn btn-outline-secondary btn-sm ms-2"><i class="fa-solid fa-rotate me-1"></i> Reset</button></div>
      </div>
      <div class="table-wrapper">
        <table class="table table-hover table-compact align-middle" id="tbl-activity">
          <thead><tr><th>Staff ID</th><th>Username</th><th>Time Stamp</th><th style="min-width:420px">Action Taken</th><th class="col-actions"><i class="fa-solid fa-gear" data-bs-toggle="tooltip" title="Actions"></i></th></tr></thead>
          <tbody></tbody>
        </table>
      </div>
      <div class="d-flex justify-content-end pt-2"><small class="text-muted" id="act-count"></small></div>
    </div>
    ...
    <div class="tab-pane fade" id="transactions" role="tabpanel" aria-labelledby="transactions-tab">
      <div class="row g-2 align-items-end mb-3 filters-row">
        ...
        <div class="col-md-3"><label class="form-label">Search</label><input type="text" id="trx-search" class="form-control form-control-sm" placeholder="Search transaction / name / plot / etc."></div>
        <div class="col-md-3 d-flex justify-content-start align-items-end"><button id="trx-filter" class="btn btn-primary btn-sm"><i class="fa-solid fa-filter me-1"></i> Apply</button><button id="trx-reset" class="btn btn-outline-secondary btn-sm ms-2"><i class="fa-solid fa-rotate me-1"></i> Reset</button></div>
      </div>
      <div class="table-wrapper">
        <table class="table table-hover table-compact align-middle" id="tbl-transactions">
          <thead>
            <tr>
              <th>Transaction ID</th><th>Burial ID</th><th>Block</th><th>Plot</th>
              <th style="min-width:210px">Interment Right</th><th style="min-width:260px">Address</th>
              <th>Contact</th><th>Email</th><th class="text-end">Payment</th>
              <th>Rental Date &amp; Time</th><th>Expiry Date</th><th style="min-width:210px">Deceased</th>
              <th>Grave</th><th>Created By</th><th>Created At</th>
      ...
      <div class="d-flex justify-content-end pt-2"><small class="text-muted" id="trx-count"></small></div>
    </div>
  </div>
</div>
