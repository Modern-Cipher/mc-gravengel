...
<style>
    .swal2-progress-bar {
        background-color: #f8f9fa; border-radius: 5px; height: 20px; width: 100%;
        overflow: hidden; border: 1px solid #ccc;
    }
    .swal2-progress-fill {
        background-color: #8a2c38; border-radius: 5px; width: 0%; height: 100%;
        ...
        <div class="col-lg-6 mb-4">
            <div class="card h-100">
                <div class="card-header">
                    <h5 class="mb-0"><i class="fas fa-download me-2"></i>Create & Download Backup</h5>
                </div>
                <div class="card-body">
                    <p>Click the button below to generate a complete backup of the database.</p>
                </div>
                <div class="card-footer text-end">
                    <button id="downloadBtn" data-url="<?= URLROOT ?>/admin/createBackup" class="btn btn-primary">
                        <i class="fas fa-play-circle me-2"></i>Start Backup
                    </button>
                </div>
            </div>
        </div>
            ...
            <div class="card h-100 border-danger">
                ...
                <div class="card-body">
                    <div class="alert alert-warning" role="alert">
                        ...
                        <p class="mb-0 mt-2">Restoring will <strong>overwrite all current data</strong>. This is irreversible.</p>
                    </div>
                    ...
                    <form id="restoreForm" action="<?= URLROOT ?>/admin/restoreBackup" method="post" enctype="multipart/form-data">
                        ...
                        <button type="submit" class="btn btn-danger w-100">
                            <i class="fas fa-hdd me-2"></i>Upload & Restore
                        </button>
                    </form>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex flex-wrap gap-2 justify-content-between align-items-center">
                    ...
                    <!-- Print without new page -->
                    <button id="printReportBtn"
                            class="btn btn-sm btn-secondary"
                            data-url="<?= URLROOT ?>/admin/printAuditHistory">
...
<iframe id="print_frame" title="print-frame" style="display:none" aria-hidden="true"></iframe>
...
<div class="row">
    <div class="col-12">
        <div class="scroll-spacer-dummy"></div>
    </div>
</div>
    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', function(e) {
            ...
            fetch(url)
                ...
                .catch(error => {
                    Swal.fire('Error', 'Could not generate backup file. ' + error.message, 'error');
                });
        });
    }
    ...
    if (restoreForm) {
        restoreForm.addEventListener('submit', function (e) {
            ...
            if (fileInput.files.length === 0) {
                Swal.fire('No File Selected', 'Please select a .sql file.', 'error');
                return;
            }
            Swal.fire({
                title: 'ARE YOU SURE?',
                ...
                preConfirm: (val) => {
                    if (val !== 'OVERWRITE') { Swal.showValidationMessage('Incorrect confirmation text.'); }
                },
                allowOutsideClick: () => !Swal.isLoading()
            }).then((result) => {
                if (result.isConfirmed) { uploadAndRestore(new FormData(restoreForm)); }
            });
        });
    }
        ...
        Swal.fire({
            ...
            willOpen: () => {
                ...
                xhr.addEventListener('error', () => {
                    Swal.fire('Network Error', 'Could not upload file.', 'error');
                });
    ...
    if (printBtn) {
        printBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // Ensure relative assets resolve correctly
                const baseHref = '<?= URLROOT ?>' + (<?= json_encode(substr(URLROOT, -1) === '/') ? '""' : '"/"' ?>);
                const baseTag = `<base href="${baseHref}">`;
                if (!/</i.test(html)) { html = '<!doctype html><html><head>'+baseTag+'</head><body>'+html+'</body></html>'; }
                else {
                    html = html.replace(/<head([^>]*)>/i, `<head$1>${baseTag}`);
                }

                // Inject auto-print (onload) script
                ...
                const idoc = doc.document || doc; // cross-compat
                idoc.open();
                idoc.write(html);
                idoc.close();

                // After print, cleanup and close Swal
                const afterPrint = () => {
                    try {
                        // Clean up iframe content to free memory
                        idoc.open(); idoc.write('<!doctype html><html><body></body></html>'); idoc.close();
                    } catch (_) {}
                    Swal.close();
                };

                // Bind afterprint on iframe window
                if (iframe.contentWindow) {
                    iframe.contentWindow.addEventListener('afterprint', afterPrint, { once: true });
                    // Fallback safety in case afterprint isn't fired
                    setTimeout(afterPrint, 6000);
                } else {
                    setTimeout(afterPrint, 2000);
                }
            } catch (err) {
                console.error(err);
                Swal.fire('Print Error', err.message || 'Unable to prepare the report for printing.', 'error');
            }
        });
    }
});
