    ...
    <div class="row">
        <div class="col-lg-6 mb-4">
            <div class="card h-100">
                ...
                <div class="card-body">
                    <p>Click the button below to generate a complete backup of the database as a <strong>.sql</strong> file.</p>
                </div>
                <div class="card-footer text-end">
                    <button id="downloadBtn" data-url="<?= URLROOT ?>/admin/createBackup" class="btn btn-primary">
                        <i class="fas fa-play-circle me-2"></i>Start Backup & Download
                    </button>
                </div>
            </div>
        </div>
            ...
            <div class="card h-100 border-danger">
                ...
                <div class="card-body">
                    <div class="alert alert-warning" role="alert">
                        <strong><i class="fas fa-exclamation-triangle"></i> EXTREME CAUTION ADVISED!</strong>
                        <p class="mb-0 mt-2">Restoring will <strong>completely overwrite all current data</strong>. This action is irreversible.</p>
                    </div>
                    <form id="restoreForm" action="<?= URLROOT ?>/admin/restoreBackup" method="post" enctype="multipart/form-data">
                        ...
                        <button type="submit" class="btn btn-danger w-100"><i class="fas fa-hdd me-2"></i>Upload & Restore Database</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="<?= URLROOT ?>/admin/printAuditHistory" target="_blank" class="btn btn-sm btn-secondary">
...
<script>
// Ang JavaScript ay mananatili, walang pagbabago.
            ...
            fetch(url)
                ...
                .then(({ blob, filename }) => {
                    ...
                    Swal.fire({
                        ...
                        icon: 'success',
                        timer: 2000,
                        didOpen: () => { setTimeout(() => { location.reload(); }, 1500); }
                    });
                })
                .catch(error => {
    ...
    if (restoreForm) {
        restoreForm.addEventListener('submit', function (e) {
            ...
            if (fileInput.files.length === 0) {
                Swal.fire('No File Selected', 'Please select a .sql file to restore.', 'error');
                return;
            }
            ...
            Swal.fire({
                ...
                preConfirm: (inputValue) => {
                    if (inputValue !== 'OVERWRITE') {
                        Swal.showValidationMessage('Incorrect confirmation text. Restore cancelled.');
                    }
                },
                allowOutsideClick: () => !Swal.isLoading()
            }).then((result) => {
                if (result.isConfirmed) {
                    uploadAndRestore(new FormData(restoreForm));
                }
            });
        });
    }
        ...
        Swal.fire({
            ...
            willOpen: () => {
                ...
                xhr.addEventListener('load', () => {
                    if (xhr.status >= 200 && xhr.status < 300) {
                        Swal.update({
                            ...
                            html: 'The database is now being restored. Please do not close this page.',
                        });
                        location.reload();
                    } else {
                ...
                xhr.addEventListener('error', () => {
                    Swal.fire('Network Error', 'Could not upload the file.', 'error');
                });
