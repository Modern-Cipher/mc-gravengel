...
(function(){
  ...
  byId('rental_date').addEventListener('change', function() {
      const disp = byId('expiry_date_display');
      const hid = byId('expiry_date');
      if (!this.value) {
          disp.value = '';
          hid.value = '';
          return;
      ...
      const mm = pad(exp.getMonth() + 1);
      const dd = pad(exp.getDate());
      const hh = pad(exp.getHours());
      const min = pad(exp.getMinutes());
      const ss = pad(exp.getSeconds());

      hid.value = `${yyyy}-${mm}-${dd} ${hh}:${min}:${ss}`;
  ...
  function bakeAddress(){
    const parts = [ byId('addr_line').value.trim(), byId('addr_brgy').value ? 'Brgy. '+byId('addr_brgy').selectedOptions[0].text : '', byId('addr_city').value ? byId('addr_city').selectedOptions[0].text : '', byId('addr_province').value ? byId('addr_province').selectedOptions[0].text : '', byId('addr_zip').value.trim() ].filter(Boolean);
  ...
  byId('submitBurial').onclick = async ()=>{
    ...
    const formatDateForDisplay = (dateString) => {
        ...
        return new Date(dateString).toLocaleString('en-US', { weekday: 'short', year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true });
    };

    const sum = document.getElementById('summaryFields');
    sum.innerHTML = `<div class="row g-2"><div class="col-6"><strong>Plot ID:</strong> ${byId('plot_id').selectedOptions[0].text}</div><div class="col-6"><strong>Burial ID:</strong> ${savedBurialId}</div><div class="col-6"><strong>First Name:</strong> ${byId('deceased_first_name').value}</div><div class="col-6"><strong>Relationship:</strong> ${byId('interment_relationship').value}</div><div class="col-6"><strong>Middle Name:</strong> ${byId('deceased_middle_name').value}</div><div class="col-6"><strong>Contact:</strong> ${byId('interment_contact_number').value}</div><div class="col-12"><strong>Email:</strong> ${byId('interment_email').value || '-'}</div><div class="col-12"><strong>Address:</strong> ${byId('interment_address').value}</div><div class="col-6"><strong>Payment Amount:</strong> ${Number(byId('payment_amount').value).toLocaleString()}</div><div class="col-6"><strong>Rental Date:</strong> ${formatDateForDisplay(byId('rental_date').value)}</div><div class="col-6"><strong>Expiry Date:</strong> ${formatDateForDisplay(byId('expiry_date').value)}</div><div class="col-12"><strong>Grave Level & Type:</strong> ${byId('grave_level').value || '-'} / ${byId('grave_type').value || '-'}</div><div class="col-12"><strong>Requirements:</strong> ${byId('requirements').value || '-'}</div></div>`;

    const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data='+encodeURIComponent(savedBurialId);
    document.getElementById('qr1').src = qrUrl; document.getElementById('qr2').src = qrUrl;
    document.getElementById('qrBurial').innerText = savedBurialId; document.getElementById('qrTxn').innerText = savedTxn;
