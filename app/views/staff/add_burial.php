...
(function(){
  ...
  if (plotSelect && parentIdInput && graveLevelSelect && graveTypeSelect) {
      plotSelect.addEventListener('change', async function() {
          ...
          try {
              if (status === 'occupied') {
                  const parentRes = await fetch(`<?= URLROOT ?>/staff/getPrimaryBurialForPlot/${plotId}`).then(r => r.json());
                  if (parentRes.ok) {
              ...
              const detailsRes = await fetch(`<?= URLROOT ?>/staff/getPlotDetailsForForm/${plotId}`).then(r => r.json());
              if (!detailsRes.ok) {
  ...
  byId('submitBurial').onclick=async()=>{
    ...
    if(r.isConfirmed){
      ...
      try{
        const rp=await fetch('<?= URLROOT ?>/staff/addBurial',{method:'POST',body:p}).then(r=>r.json());
  ...
  const pu=(t,id)=>`<?= URLROOT ?>/staff/${t}/${encodeURIComponent(id)}?autoprint=1`;
  ...
  byId('finishBtn').onclick=()=>Swal.fire({icon:'success',title:'Saved!',timer:1500,showConfirmButton:false}).then(()=>window.location='<?= URLROOT ?>/staff/burialRecords');
