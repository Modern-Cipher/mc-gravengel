<?php
/**
 * app/views/staff/contact_us.php
 * Staff-facing “Contact Us” with QR + modal report form (SweetAlert + regex validation).
 * Uses staff header/footer if available; falls back to admin/generic includes.
 */

// --- HEADER include (prefer staff, then admin, then generic) ---
if (file_exists(APPROOT . '/views/includes/staff_header.php')) {
    require APPROOT . '/views/includes/staff_header.php';
} elseif (file_exists(APPROOT . '/views/includes/admin_header.php')) {
    require APPROOT . '/views/includes/admin_header.php';
} elseif (file_exists(APPROOT . '/views/includes/header.php')) {
    require APPROOT . '/views/includes/header.php';
}

$data['title'] = 'Contact Us (Staff)';

// Developer emails (same set you used in admin)
...
.scroll-spacer-dummy{height:600px;opacity:0;visibility:hidden;pointer-events:none;}
...
<?php
// --- FOOTER include (prefer staff, then admin, then generic) ---
if (file_exists(APPROOT . '/views/includes/staff_footer.php')) {
    require APPROOT . '/views/includes/staff_footer.php';
} elseif (file_exists(APPROOT . '/views/includes/admin_footer.php')) {
    require APPROOT . '/views/includes/admin_footer.php';
} elseif (file_exists(APPROOT . '/views/includes/footer.php')) {
...
(function () {
  ...
  function init() {
    ...
    const QR_COLOR_DARK = '#000000', QR_COLOR_LIGHT = '#ffffff';

    // Fill report modal header
    ...
    document.getElementById('developerReportForm').addEventListener('submit', function(e){
      ...
      fetch('<?php echo URLROOT; ?>/staff/sendDeveloperReport',{ // staff route; controller already aligned
    ...
    document.querySelectorAll('.qr-popover').forEach(function(item){
      item.addEventListener('click', function(e){
        ...
        new QRCode(qrBox,{text:qr_content,width:220,height:220,colorDark:QR_COLOR_DARK,colorLight:QR_COLOR_LIGHT,correctLevel:QRCode.CorrectLevel.H});
