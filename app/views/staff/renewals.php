...
document.addEventListener('DOMContentLoaded', () => {
            ...
            if (result.ok) {
                if (url.includes('processRenewal')) {
                    themedSwal.fire({
                        ...
                        if (action.isDenied) {
                            window.open(`<?= URLROOT ?>/staff/printContract/${result.burial_id}?autoprint=1`, '_blank');
    ...
    document.getElementById('confirmRenewalBtn').addEventListener('click', () => {
        ...
        handleFormSubmit('<?= URLROOT ?>/staff/processRenewal', new FormData(form));
    });
    document.getElementById('vacateBtn').addEventListener('click', async () => {
        ...
        if (result.isConfirmed) {
            ...
            handleFormSubmit('<?= URLROOT ?>/staff/processVacate', formData);
        }
    });
    ...
    if (generateReportBtn) {
        generateReportBtn.addEventListener('click', () => {
            ...
            iframe.src = '<?= URLROOT ?>/staff/printRenewalHistory';
