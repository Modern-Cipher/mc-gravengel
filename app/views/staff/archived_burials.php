...
<style>
  ...
  .actions .i-view{ color:#0d6efd } .actions .i-print{ color:#0dcaf0 } .actions .i-qr{ color:#6c757d }
...
<div class="records-wrap">
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex align-items-center">
      <input id="tableSearch" type="search" class="form-control form-control-sm search-input" placeholder="Search name, burial id, plot, email…">

      <a href="<?= URLROOT ?>/staff/burialRecords" class="btn btn-secondary btn-sm ms-2 action-btn">
          ...
          <?php foreach ($data['records'] as $r): ?>
            ...
            <tr data-burial-id="<?= htmlspecialchars($r->burial_id) ?>">
              ...
              <td class="text-center actions">
                ...
                <!-- If you want staff to restore, wire a staff endpoint -->
...
<div class="modal fade" id="viewModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body"><div id="viewBody" class="table-responsive"></div></div>
...
(() => {
  ...
  table.addEventListener('click', async (e) => {
    ...
    const tr = control.closest('tr'); currentBurialId = tr.dataset.burialId;
  ...
  function showQr(id){
    document.getElementById('qrImg').src = 'https://api.qrserver.com/v1/create-qr-code/?size=220x220&data='+encodeURIComponent(id);
    document.getElementById('qrMeta').innerText=id;
    qrModal.show();
  }

  async function confirmRestore(id, row){
    const ans = await Swal.fire({ icon:'question', title:'Restore this record?', text:'It will return to the active Burial Records.', showCancelButton:true, confirmButtonText:'Restore', confirmButtonColor:'#7b1113' });
    if(!ans.isConfirmed) return;

    // If restore is admin-only in your backend, call admin endpoint. Otherwise change to staff/restoreBurial.
    const res = await fetch('<?= URLROOT ?>/admin/restoreBurial/'+encodeURIComponent(id), {
      method:'POST', headers:{'X-Requested-With':'XMLHttpRequest'}
    }).then(r=>r.json()).catch(()=>({ok:false}));

    if(res.ok){ row.style.transition='opacity .2s'; row.style.opacity='0'; setTimeout(()=>row.remove(),200); toast('Restored','success'); }
    else{ Swal.fire({icon:'error',title:'Restore failed',text:res.message||'Please try again.'}); }
  }
